...
# --- demo fixture bake / replay -------------------------------------------


def apply_demo_enrichment(space: Space, enrichment: dict) -> None:
    if not enrichment:
        return
    space.seed.ai_summary = enrichment.get("seed_summary")
    space.summary = enrichment.get("space_summary")
    space.position_summary = enrichment.get("position_summary")
    space.incumbents_summary = enrichment.get("incumbents_summary")
    space.established_summary = enrichment.get("established_summary")
    if enrichment.get("space_name"):
        space.name = enrichment["space_name"]
    if enrichment.get("space_themes"):
        space.themes = enrichment["space_themes"]
    blurbs = enrichment.get("blurbs", {})
    summaries = enrichment.get("summaries", {})
    for c in [space.seed, *space.companies]:
        if c.domain in blurbs:
            c.ai_blurb = blurbs[c.domain]
        if not c.is_seed and c.domain in summaries:
            c.ai_summary = summaries[c.domain]


def extract_enrichment(space: Space) -> dict:
    return {
        "space_name": space.name,
        "space_themes": space.themes,
        "seed_summary": space.seed.ai_summary,
        "space_summary": space.summary,
        "position_summary": space.position_summary,
        "incumbents_summary": space.incumbents_summary,
        "established_summary": space.established_summary,
        "blurbs": {c.domain: c.ai_blurb for c in [space.seed, *space.companies] if c.ai_blurb},
        "summaries": {c.domain: c.ai_summary for c in space.companies if c.ai_summary},
    }
